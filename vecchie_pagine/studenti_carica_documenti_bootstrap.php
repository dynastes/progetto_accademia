...
<script>
    function showDiv1() {
        var my_disply = document.getElementById('Altro').style.display;
        if(my_disply == "block"){
              document.getElementById('Altro').style.display = "none";
			  document.getElementById('btn1').innerHTML = "Per aggiungere delle note clicca qui";
		      
		}
        else{
              document.getElementById('Altro').style.display = "block";
			  document.getElementById('btn1').innerHTML = "Ho cambiato idea, non voglio inserire una nota";
		}
				...
				<div id="avvisi">
					<?php
						if(isset($_FILES['FileUtente'])){
							...
							$stringasql="INSERT INTO studenti_documenti_caricati (Id_studente, Percorso_file, Nome_file, Data_caricamento, Tipo,Altro) VALUES(".$utente->id.",'".$directory."', '".$_FILES['FileUtente']['name']."',SYSDATE(), ".$tipoFile.",'".$Altro."')";
							...
							echo 'Per caricare un ulteriore file, <a href="studenti_carica_documenti_bootstrap.php">cliccare qui</a>';
						}else{
							...
							<form width:60%;" action="studenti_carica_documenti_bootstrap.php" enctype="multipart/form-data" method="POST">
								...
								<input class="btn btn-info" style="float:right;" type="submit" value="Invia file/documento">
							</form>
							<button id="btn1" name="btn1" onclick="showDiv1()" style="float:center;"/>Per aggiungere delle note clicca qui</button>';
						}
